...
def remove_outliers(
    dataframe: pd.DataFrame, percentiles: list, column_names: list
) -> pd.DataFrame:
    df = dataframe.copy()

    outlier_transformer = OutliersRemover(
        percentile_values=percentiles, col_subset=column_names
    )
...
def main():
    ...
    for filename in sys.argv[1:]:
        ...
        if filename == "train.csv":
            ...
            outlier_transformer = remove_outliers(
                dataframe=X, percentiles=percentiles, column_names=COLUMN_NAMES
            )

            # save the transformer
            save_transformer(
                path=save_transformers_path / "outliers.joblib",
                object=outlier_transformer,
            )

            # transform the data
            df_without_outliers = transform_data(
                transformer=outlier_transformer, data=X
            )
            ...
            save_transformer(
                path=save_transformers_path / "preprocessor.joblib", object=preprocessor
            )
            ...
            y_trans = transform_data(
                transformer=output_transformer, data=y.values.reshape(-1, 1)
            )
            ...
            save_transformer(
                path=save_transformers_path / "output_transformer.joblib",
            ...
            # load the transfomer
            outlier_transformer = joblib.load(
                save_transformers_path / "outliers.joblib"
            )
            df_without_outliers = transform_data(
                transformer=outlier_transformer, data=X
            )
            ...
            output_transformer = joblib.load(
                save_transformers_path / "output_transformer.joblib"
            )
            ...
            df_without_outliers = transform_data(
                transformer=outlier_transformer, data=df
            )
            ...
            # transform the data
            X_trans = transform_data(transformer=preprocessor, data=df)
