...
def make_datetime_features(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    modify_logger.save_logs(msg=f'The number of columns increased by 4 verify={transformed_number_of_columns == (original_number_of_columns + 5 - 1)}')
...
def remove_passengers(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    # filter out rows which matches exactly the passengers in the list
